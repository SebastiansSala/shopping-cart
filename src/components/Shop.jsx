...
const Shop = ({ setShowCart, showCart, setShopList, shopList, counter, setCounter}) => {
  ...
  return (
    <div>
      ...
      <div className="flex-1 flex flex-wrap gap-10 mt-40 justify-center container mx-auto">
        {keyboardList.map((keyboard, index) => (
          <div className="rounded-xl w-80 h-90 p-10 shadow-md" key={index}>
            <img
              src={keyboard.img}
              className="rounded w-full h-40 object-cover"
            ></img>
            <p className="text-black mt-6 text-3xl font-bold">
              Name{keyboard.name}
            </p>
            <p className="text-black mt-6 text-2xl">arst{keyboard.desc}</p>
